...
public class TaqueroAI : CharacterStats
{
    ...
    protected override void Start()
    {
        ...
        if (player == null)
        {
            ...
            {
                Debug.LogError("TaqueroAI: No se encontr� al jugador.");
            }
        }
    ...
    // Decide qu� hacer (moverse o disparar)
    void Update()
    {
        if (player == null) return;

        // Si est� en el cooldown del ataque de DISPARO, no hace nada
        if (isAttacking)
        {
    ...
    {
        isAttacking = true; // Previene que se mueva y dispare m�s
